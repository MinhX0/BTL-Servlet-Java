...
public class ProductDAO {
    ...
    public Product getById(int id) {
        try (Session session = HibernateUtil.getSession()) {
            Query<Product> q = session.createQuery(
                    "SELECT p FROM Product p JOIN FETCH p.category WHERE p.id = :id AND p.active = true",
                    Product.class
            );
            q.setParameter("id", id);
            Product p = q.uniqueResult();
            if (p == null) {
                // Fallback direct get (may return inactive); we keep it null to hide inactive in UI
                return null;
            }
            return p;
    ...
    // Fetch by id including inactive (for historical views / order details)
    public Product getByIdIncludingInactive(int id) {
        try (Session session = HibernateUtil.getSession()) {
            Query<Product> q = session.createQuery(
            ...
            System.out.println("Error fetching product by id (including inactive): " + e.getMessage());
            e.printStackTrace();
            return null;
        }
    }
    ...
    public List<Product> listAll() {
        try (Session session = HibernateUtil.getSession()) {
            return session.createQuery(
                    "SELECT p FROM Product p LEFT JOIN FETCH p.category WHERE p.active = true ORDER BY p.dateAdded DESC, p.id DESC",
                    Product.class
            ).getResultList();
        } catch (HibernateException e) {
    ...
    public List<Product> listByCategory(int categoryId) {
        try (Session session = HibernateUtil.getSession()) {
            Query<Product> q = session.createQuery(
                    "SELECT p FROM Product p JOIN FETCH p.category WHERE p.category.id = :cid AND p.active = true ORDER BY p.dateAdded DESC, p.id DESC",
                    Product.class
            );
    ...
    public List<Product> findByName(String name) {
        ...
        try (Session session = HibernateUtil.getSession()) {
            Query<Product> q = session.createQuery(
                    "SELECT p FROM Product p LEFT JOIN FETCH p.category WHERE p.active = true AND lower(p.name) LIKE :pattern ORDER BY p.dateAdded DESC, p.id DESC",
                    Product.class
            );
    ...
    public List<Product> search(Integer categoryId, String name) {
        StringBuilder hql = new StringBuilder("SELECT p FROM Product p LEFT JOIN FETCH p.category WHERE p.active = true");
    ...
    public long searchCount(Integer categoryId, String name) {
        StringBuilder hql = new StringBuilder("SELECT COUNT(p.id) FROM Product p WHERE p.active = true");
        boolean hasName = name != null && !name.trim().isEmpty();
        if (categoryId != null) {
            hql.append(" AND p.category.id = :cid");
        }
    ...
    public List<Product> listOnSale(int limit) {
        try (Session session = HibernateUtil.getSession()) {
            String hql = "SELECT p FROM Product p LEFT JOIN FETCH p.category " +
                    "WHERE p.active = true AND COALESCE(p.salePrice, 0) > 0 AND COALESCE(p.basePrice, 0) > COALESCE(p.salePrice, 0) " +
    ...
    public long countAll() {
        try (Session session = HibernateUtil.getSession()) {
            Query<Long> q = session.createQuery("SELECT COUNT(p.id) FROM Product p WHERE p.active = true", Long.class);
