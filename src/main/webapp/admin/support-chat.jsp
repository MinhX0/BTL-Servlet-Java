...
(function(){
  ...
  function poll(){const url = ctx+"/chat/poll?after="+lastTs+(currentTarget?"&customer="+encodeURIComponent(currentTarget):""); fetch(url).then(r=>r.json()).then(data=>{renderCustomers(Array.from(data.activeCustomers||[])); (data.messages||[]).forEach(m=>{lastTs=Math.max(lastTs,m.ts); const mine=m.fromSeller; const who=m.fromSeller?('Tôi<span class=\"seller-tag\">SELLER</span>'):escapeHtml(m.from); addMsg(mine?'me':'other','<strong>'+who+':</strong> '+escapeHtml(m.content));}); setTimeout(poll,1500);}).catch(()=>setTimeout(poll,3000));}
  poll();
